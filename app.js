...
function ligarModalNovaAluna() {
  ...
  document.getElementById('btnSalvarNovaAluna').addEventListener('click', function () {
    ...
    Api.salvarGestaoAlunaComResposta(dados).then(function (resultado) {
      btn.disabled = false;
      btn.textContent = 'Cadastrar';

      if (!resultado || !resultado.id) {
        mostrarToast('Cadastrado, mas não consegui confirmar o ID. Atualize a lista pra conferir.');
      } else {
        mostrarToast((resultado.jaExistia ? 'Esta aluna já existia — atualizei o cadastro. ' : 'Aluna cadastrada! ') + 'ID ' + resultado.id);

        if (enviarContrato && telefone) {
          const whats = telefoneParaWhats(telefone);
          if (whats) {
            const separador = CONFIG.linkContrato.indexOf('?') === -1 ? '?' : '&';
            const linkContratoAluna = CONFIG.linkContrato + separador + 'nome=' + encodeURIComponent(nome);
            const msg = 'Oi, ' + primeiroNome(nome) + '! Que bom ter você como aluna 💪 Antes de começarmos, preciso que você leia e aceite o contrato: ' + linkContratoAluna;
            Api.enviarAcao({ tipo: 'gerar_contrato', nome: nome }).then(function () {
      ...
      btn.disabled = false;
      btn.textContent = 'Cadastrar';
      mostrarToast('Não consegui cadastrar agora. Verifique sua internet e tente de novo.');
    });
  });
}
