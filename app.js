...
const TITULOS_SECAO = {
  ...
  alunas: ['Gestão de alunas', 'Todas as alunas, fichas e cadastros manuais'],
  checkins: ['Check-ins', 'Histórico de check-ins semanais'],
  financeiro: ['Financeiro', 'Pagamentos, planos e renovações'],
  emails: ['E-mails', 'Modelos de e-mail para suas alunas'],
  whatsapp: ['WhatsApp', 'Modelos de mensagem e compartilhamento do seu número'],
...
function mostrarSecao(nome) {
  ...
  if (nome === 'checkins') renderCheckins();
  if (nome === 'financeiro') renderFinanceiro();
  if (nome === 'emails') renderModelosEmail();
  if (nome === 'whatsapp') renderModelosWhats();
}
...
function renderDashboard() {
  ...
  document.getElementById('dashboardStats').innerHTML = [
    ['Alunas', total], ['Ativas', ativas], ['Inativas', inativas],
    ['Check-ins pendentes', checkinsPendentes], ['Pagamentos pendentes', pagamentosPendentes],
    ['Fotos pendentes', fotosPendentes], ['Renovações em 7 dias', reavaliacoesProximas]
  ].map(function (s) {
    return '<div class="stat-card"><div class="num">' + s[1] + '</div><div class="lbl">' + s[0] + '</div></div>';
  }).join('');
  ...
  visiveis.forEach(function (a) {
    ...
    if (diasCheckin === null || diasCheckin >= CONFIG.diasAlertaCheckin) {
      alertas.push('⚠️ <b>' + nome + '</b> está ' + (diasCheckin === null ? 'sem nenhum check-in registrado' : 'há ' + diasCheckin + ' dias sem check-in') + '.');
    }
    ...
    if (diasFoto === null || diasFoto >= CONFIG.diasAlertaFotos) {
      alertas.push('⚠️ <b>' + nome + '</b> precisa atualizar as fotos de avaliação.');
    }
    const g = gestaoDe(a);
    const diasRen = diasAte(g.proximaRenovacao);
    if (g.pagamento === 'Devendo') {
      alertas.push('💳 <b>' + nome + '</b> está com pagamento pendente' + (g.valor ? ' (R$ ' + g.valor + ')' : '') + '.');
    } else if (diasRen !== null && diasRen <= 7) {
      alertas.push('📅 <b>' + nome + '</b> precisa renovar a consultoria em ' + (diasRen < 0 ? 'atraso' : diasRen + ' dia(s)') + '.');
    }
    if (parseInt(a['PAR-Q (positivos)']) > 0) {
      alertas.push('🩺 <b>' + nome + '</b> sinalizou risco no PAR-Q — vale confirmar liberação médica.');
    }
  });
...
function abrirModalAluna(aluna) {
  ...
  document.getElementById('modalAlunaSub').textContent = aluna.origem === 'manual' ? 'Cadastro manual, sem ficha de anamnese' : 'Nível ' + (aluna['Nível'] || '—') + ' · IMC ' + (aluna['IMC'] || '—');
...
function renderTabCheckins(a) {
  ...
  el.innerHTML = '<table class="data-table"><thead><tr><th>Data</th><th>Peso</th><th>Semana</th><th>Energia</th><th>Dor</th><th>Observação</th></tr></thead><tbody>' +
...
function renderCheckins() {
  ...
  corpo.innerHTML = todos.length ? todos.slice(0, 100).map(function (c) {
    return '<tr><td>' + formatarDataHora(c['Data/Hora']) + '</td><td>' + c._nome + '</td><td>' + (c['Peso'] || '—') + '</td><td>' + (c['Semana'] || '—') + '</td><td>' + (c['Energia'] ?? '—') + '/10</td><td>' + (c['Dor'] === 'Sim' ? '⚠ ' + (c['Onde a Dor'] || 'sim') : 'Não') + '</td></tr>';
  }).join('') : '<tr><td colspan="6" style="text-align:center; color:var(--ink-soft);">Nenhum check-in registrado ainda.</td></tr>';
}
...
/* ============================================================
   ...
   ============================================================ */
function renderModelosEmail() {
  const el = document.getElementById('listaModelosEmail');
  const chaves = { boasVindas: 'Boas-vindas', lembreteSemanal: 'Lembrete semanal', solicitarFotos: 'Solicitação de fotos', renovacao: 'Renovação da consultoria' };
  el.innerHTML = Object.keys(chaves).map(function (k) {
    const m = CONFIG.modelosEmail[k];
    return '<div class="template-card"><div class="tit">' + chaves[k] + '</div>' +
      '<div class="form-field"><label>Assunto</label><input type="text" data-modelo-email-assunto="' + k + '" value="' + m.assunto.replace(/"/g, '&quot;') + '"></div>' +
      '<div class="form-field"><label>Corpo</label><textarea data-modelo-email-corpo="' + k + '">' + m.corpo + '</textarea></div>' +
    '</div>';
  }).join('');

  document.getElementById('btnSalvarModelosEmail').onclick = function () {
    Object.keys(chaves).forEach(function (k) {
      CONFIG.modelosEmail[k].assunto = document.querySelector('[data-modelo-email-assunto="' + k + '"]').value;
      CONFIG.modelosEmail[k].corpo = document.querySelector('[data-modelo-email-corpo="' + k + '"]').value;
    });
    salvarConfig(CONFIG);
    mostrarToast('Modelos de e-mail salvos!');
  };
}

function renderModelosWhats() {
  const el = document.getElementById('listaModelosWhats');
  const chaves = { boasVindas: 'Boas-vindas', lembreteSemanal: 'Lembrete semanal', solicitarCheckin: 'Solicitação de check-in', solicitarFotos: 'Solicitação de fotos', lembretePagamento: 'Lembrete de pagamento', motivacao: 'Motivação' };
  el.innerHTML = Object.keys(chaves).map(function (k) {
    return '<div class="template-card"><div class="tit">' + chaves[k] + '</div>' +
      '<div class="form-field"><textarea data-modelo-whats="' + k + '">' + CONFIG.modelosWhatsapp[k] + '</textarea></div></div>';
  }).join('');

  document.getElementById('btnSalvarModelosWhats').onclick = function () {
    Object.keys(chaves).forEach(function (k) {
      CONFIG.modelosWhatsapp[k] = document.querySelector('[data-modelo-whats="' + k + '"]').value;
    });
    salvarConfig(CONFIG);
    mostrarToast('Modelos de WhatsApp salvos!');
  };
