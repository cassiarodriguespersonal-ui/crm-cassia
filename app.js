...
function mostrarSecao(nome) {
  ...
  if (nome === 'ferramentas') { renderCompartilhar(); renderModelosEmail(); renderModelosWhats(); renderEnvioFerramenta(); }
}
...
function renderTabCheckins(a) {
  ...
  el.innerHTML = '<table class="data-table"><thead><tr><th>Data</th><th>Tipo</th><th>Peso</th><th>Avaliação</th><th>Energia</th><th>Dor</th><th>Observação</th></tr></thead><tbody>' +
    lista.map(function (c) {
      return '<tr><td>' + formatarDataHora(c['Data/Hora']) + '</td><td>' + (c['Tipo'] || 'Semanal') + '</td><td>' + (c['Peso'] || '—') + '</td><td>' + (c['Semana'] || '—') + '</td><td>' + (c['Energia'] ?? '—') + '/10</td><td>' + (c['Dor'] === 'Sim' ? '⚠ ' + (c['Onde a Dor'] || 'sim') : 'Não') + '</td><td>' + (c['Observação'] || '—') + '</td></tr>';
    }).join('') + '</tbody></table>';
}
