...
function renderDashboard() {
  ...
  visiveis.forEach(function (a) {
    ...
    if (!ob.completo) emOnboarding.push({ nome: a['Nome'], telefone: a['Telefone'], ob: ob });
  });
  ...
  if (emOnboarding.length) {
    ...
    document.getElementById('dashboardOnboarding').innerHTML = emOnboarding.map(function (o, indice) {
      ...
      return '<div class="care-card">' +
        ...
        '<div class="acao">' + (whats ? '<button class="btn btn-sm btn-accent" data-onboarding-msg="' + indice + '">💬 Lembrar</button>' : '') + '</div>' +
      '</div>';
    }).join('');
    document.querySelectorAll('[data-onboarding-msg]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        const o = emOnboarding[parseInt(btn.getAttribute('data-onboarding-msg'), 10)];
        const whats = telefoneParaWhats(o.telefone);
