  ...
  lista.innerHTML = filtradas.map(function (a, indice) {
    ...
    return '<div class="aluna-card' + arquivadaClasse + '">' +
      '<div class="aluna-info">' +
        '<div class="nome">' + (a['Nome'] || '—') + (a['ID'] ? ' <span style="color:var(--ink-faint); font-weight:400; font-size:.78rem;">#' + a['ID'] + '</span>' : '') + '</div>' +
...
function renderTabAcoes(a) {
  ...
  if (contrato && contrato['Status'] === 'Aceito') {
    ...
    html += '<div style="background:#DEEAE1; color:#3E6650; border-radius:8px; padding:.7rem .9rem; font-size:.85rem; margin-bottom:1.4rem;">' +
      '✓ Aceito em ' + dataAceite + (contrato['Hora Aceite'] ? ' às ' + contrato['Hora Aceite'] : '') +
      '</div>';
  } else {
  ...
  html += '<h3 style="font-size:.95rem; margin-bottom:.7rem;">WhatsApp</h3><div style="display:flex; gap:.5rem; flex-wrap:wrap; margin-bottom:1.4rem;">';
  if (!whats) {
