...
function calcularRisco(a) {
  ...
  let nivel = 'excelente';
  if (urgente || pontuacao >= 5) nivel = 'critico';
  else if (pontuacao >= 3) nivel = 'em_risco';
  else if (pontuacao >= 1) nivel = 'atencao';
...
function renderDashboard() {
  ...
  visiveis.forEach(function (a) {
    ...
    if (r.sinais.length) {
      candidatas.push({ nome: a['Nome'], telefone: a['Telefone'], sinais: r.sinais, urgente: r.urgente, nivel: r.nivel, diasCheckin: r.diasCheckin });
    }
  });
...
function renderTabInteracoes(a) {
  ...
  if (!lista.length) {
    el.innerHTML = '<p style="color:var(--ink-soft);">Nenhuma interação registrada ainda.</p>' +
      '<div style="margin-top:1.4rem;"><button class="btn btn-accent" id="btnRegistrarInteracao">+ Registrar interação</button></div>';
  } else {
    el.innerHTML = '<div style="margin-bottom:1.4rem;">' + lista.map(function (i) {
      return '<div style="display:flex; gap:.8rem; padding:.7rem 0; border-bottom:1px solid var(--line-soft);">' +
    ...
    '<button class="btn btn-accent" id="btnRegistrarInteracao">+ Registrar nova interação</button>';
  }

  document.getElementById('btnRegistrarInteracao').addEventListener('click', function () {
    mostrarModalRegistrarInteracao(a['Nome']);
  });
}

function mostrarModalRegistrarInteracao(nomeAluna) {
  const tipo = prompt('Tipo de interação (ex.: Mensagem, Ligação, Check-in cobrado, Outro):', 'Mensagem');
  if (tipo === null) return;
  ...
  enviarDadosInteracao({
    nome: nomeAluna,
    tipoInteracao: tipo || 'Outro',
    observacao: obs || ''
  });
}

function enviarDadosInteracao(dados) {
  const form = document.createElement('form');
  form.method = 'POST';
  form.target = 'hiddenFrameApi';
  form.action = CONFIG.urlBackend;
  form.innerHTML = '<input type="hidden" name="payload" value="' + JSON.stringify(Object.assign({}, dados, { tipo: 'interacao' })).replace(/"/g, '&quot;') + '">';
